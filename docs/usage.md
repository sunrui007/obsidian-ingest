...
需要 Python 3.10+。只将仓库内的 [skills/obsidian-ingest](../skills/obsidian-ingest) 目录复制到宿主的技能目录；不要复制仓库根目录、`docs/` 或 `tests/` 作为技能。运行所需的 `SKILL.md`、脚本、配置、依赖清单及 `references/` 已包含在该技能目录中。

以下安装和配置命令均在**复制后的技能目录**中运行：
...
完整指令见 [SKILL.md](../skills/obsidian-ingest/SKILL.md)，命令、配置项和格式恢复流程见 [references/workflow.md](../skills/obsidian-ingest/references/workflow.md)。
...
测试保留在仓库根目录的 `tests/`，无需安装到宿主。以下命令从**仓库根目录**运行；PDF 测试还需要 `reportlab`：

```sh
python -m pip install -r skills/obsidian-ingest/requirements.txt reportlab
